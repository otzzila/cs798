...
#define NUMBER_OF_ALWAYS_EMPTY_EPOCH_BAGS 0 // 3 for range query support
    ...
    public:
        ...
        int opsSinceRead;
        int debug_announcedEpoch;
        int debug_epochbag_announcedEpoch[NUMBER_OF_EPOCH_BAGS];  // used for debug timeline printing...
        ThreadData() {}
    private:
        PAD;
    };
...
private:
    long long getSizeInNodesForThisThread(int tid) {
        ...
        for (int j=0;j<NUMBER_OF_EPOCH_BAGS;++j) {
            if (threadData[tid].epochbags[j]) {
                sum += threadData[tid].epochbags[j]->computeSizeFast();
            }
        }
        return sum;
    }
...
#       endif
        ...
        numLeftover += freeable->computeSizeFast();
        ...
        // if (tid < 96) {
            ...
            threadData[tid].debug_epochbag_announcedEpoch[nextIndex] = threadData[tid].debug_announcedEpoch;                 // update epoch to which epochbags[nextIndex] corresponds. // works with experiments/timelines/daewoo_thesis as of 2023-03-27

            // TIMELINE_END_Llu(tid, "rotateEpochBags", threadData[tid].debug_epochbag_announcedEpoch[nextIndex]); // confirmed to work 2023-03-26 // last arg prints whichever epoch's bag is being freed
            //threadData[tid].debug_epochbag_announcedEpoch[nextIndex] = threadData[tid].debug_announcedEpoch; // update epoch to which epochbags[nextIndex] corresponds.

            // TIMELINE_END_Llu(tid, "rotateEpochBags", numLeftover); // confirmed to work 2023-03-26 (note: numLeftover was not used at this point)
    ...
    inline bool startOp(const int tid, void * const * const reclaimers, const int numReclaimers, const bool readOnly = false) {
        ...
        if (readEpoch != ann /* invariant: ann is not quiescent */) {
            ...
            threadData[tid].debug_announcedEpoch = readEpoch; // for debug timeline printing (this is being passed to rotateAllEpochBags here!). this is the new announced epoch to store in debug_epochbags_announcedEpoch[nextIndex].
            rotator.rotateAllEpochBags(tid, reclaimers, 0);
...
#   endif
    ...
    reclaimer_debra(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL)
        ...
        for (int tid=0;tid<numProcesses;++tid) {
            ...
            for (int i=0;i<NUMBER_OF_EPOCH_BAGS;++i) {
                threadData[tid].epochbags[i] = NULL;
                threadData[tid].debug_epochbag_announcedEpoch[i] = 0;
            }
#ifdef DEAMORTIZE_FREE_CALLS
